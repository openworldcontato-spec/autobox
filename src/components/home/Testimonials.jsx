      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <div className="flex items-center gap-3 border-t border-white/10 pt-4">
                ...
                <div className="ml-auto">
                  <span className="text-gold text-xs font-medium">{t.car}</span>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="lg:hidden">
          ...
          <div className="flex justify-center gap-3 mt-6">
            <button onClick={() => setCurrent(c => (c - 1 + testimonials.length) % testimonials.length)} className="w-9 h-9 glass rounded-full flex items-center justify-center text-white cursor-pointer" aria-label="Previous">
              <ChevronLeft className="w-4 h-4" />
            </button>
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full transition-all cursor-pointer ${i === current ? "bg-gold w-6" : "bg-white/30"}`} aria-label={`Testimonial ${i + 1}`} />
            ))}
            <button onClick={() => setCurrent(c => (c + 1) % testimonials.length)} className="w-9 h-9 glass rounded-full flex items-center justify-center text-white cursor-pointer" aria-label="Next">
